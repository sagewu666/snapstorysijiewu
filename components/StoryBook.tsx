...
export const StoryBook: React.FC<StoryBookProps> = ({ story, onFinish, kidProfile, onWordAdded }) => {
  ...
  // --- AUDIO REFS ---
  // FIXED: Removed audioContextRef to avoid limit exhaustion. Use audioManager.getAudioContext() instead.
  ...
  const playGeminiAudioData = async (base64Audio: string, textSegments: {clean: string}[]) => {
      ...
      try {
          // FIXED: Use the shared AudioContext from audioManager to prevent context limit errors
          ...
          // Create buffer with correct sample rate (usually 24000 for Gemini TTS)
          // Web Audio API will automatically handle resampling if ctx.sampleRate is different
