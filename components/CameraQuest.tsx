...
import { playClick, playSuccess, playError, playPop, playFanfare, playDelete, playPing, playToggle, playMagic } from '../utils/soundUtils';
      ...
      <div className="flex-1 min-h-0 p-3 flex flex-col pb-[env(safe-area-inset-bottom)]">
        ...
        {items.length > 0 && (
            <div className="h-20 mt-3 flex gap-2 overflow-x-auto px-1 shrink-0 scrollbar-hide items-center justify-center">
                {items.map((item) => (
                <div key={item.id} onClick={() => { if(!isProcessing) { playPop(); setPreviewItem(item); } }} className="min-w-[60px] w-14 h-14 bg-white rounded-xl shadow-md border border-slate-100 relative animate-pop-in shrink-0 p-0.5 cursor-pointer">
